...
if __name__ == '__main__':
    ...
    mir_name_list = find_tags('poloidal', common_signal) + \
        find_tags('toroidal Mir', common_signal)
    sxr_name_list = find_tags('soft', common_signal)
    axuv_name_list = find_tags('AXUV', common_signal)
    basic_name_list = list(set(common_signal) -
                           set(mir_name_list + sxr_name_list + axuv_name_list))
    # choose machine for processing and get machine tags by common name list
    machine_name = 'J-TEXT'
    basic_machine_tags = get_machine_tags(
        machine_name, basic_name_list, df_signal)
    mir_machine_tags = get_machine_tags(machine_name, mir_name_list, df_signal)
    # %%
    source_file_repo = FileRepo('.//data//jtext_data//$shot_2$00//')
    ...
    for shot in shot_list:
        ...
        last_time = list(source_file_repo.read_labels(
            shot, ['DownTime']).values())
    ...
    for signal_name in mir_name_list:
        ...
        processed_shotset = processed_shotset.process(
            ...
            output_tags=[["fft_amp_{}".format(
                mir_tag), "fft_fre_{}".format(mir_tag)]],
            save_repo=train_file_repo)
    ...
    # tags coming from process should be included like fft and so on.
